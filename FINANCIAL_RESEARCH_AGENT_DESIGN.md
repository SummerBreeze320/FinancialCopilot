...
## 9. 用户体系、金融实名认证 (KYC)、投资画像与 Spring Security RBAC 权限中心

为构建合规、安全、千人千面的企业级投研 Copilot，系统建立了完备的用户身份与权限架构：
*   **统一认证与 RBAC 权限控制**：基于 Spring Security 响应式 WebFlux 与无状态 JWT，提供 `ROLE_ADMIN`（平台运营与模型管理）、`ROLE_ANALYST`（专业买方 VIP/无限深度推理配额）与 `ROLE_USER`（普通个人投资者）的三层角色体系；
*   **金融合规实名认证 (KYC)**：满足投资者适当性管理要求，支持证件加密存储、哈希判重（SHA-256）与脱敏显示；
*   **用户投资画像与风险偏好建模 (User Investment Persona)**：全面覆盖投资者的风险等级（C1-C5）、目标年化、最大承受回撤与偏好板块风格，**并将画像作为核心事实数据无缝注入投研 Agent 黑板，定制千人千面的资产配置方案**；
*   **开户自动赠金闭环**：新用户注册自动在钱包中心开户并赠送 10,000 体验算力点。
