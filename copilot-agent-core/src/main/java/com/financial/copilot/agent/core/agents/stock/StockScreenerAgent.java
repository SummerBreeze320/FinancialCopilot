...
public class StockScreenerAgent {
    private final AgentScopeAgentFactory factory; private final StockScreeningTool screening;
    public StockScreenerAgent(AgentScopeAgentFactory factory, StockScreeningTool screening) {this.factory=factory;this.screening=screening;}

    public Artifact<String> execute(GraphNode node, NodeInput input, NodeExecutionContext context) {
        Toolkit toolkit = new Toolkit(); toolkit.registerTool(new Tools(screening));
    ...
    static final class Tools {
        private final StockScreeningTool tool; Tools(StockScreeningTool tool){this.tool=tool;}
        @Tool(name="screen_stocks",description="按结构化条件筛选真实股票",readOnly=true)
        public String screen(@ToolParam(name="exchange",description="交易所",required=false)String exchange,
            ...
            return tool.screenStocks(new StockScreeningCriteria(exchange,sector,marketCap,pe,pb,roe,dividend,sortBy,sortOrder,limit));}
    }
}
